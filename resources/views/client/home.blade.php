    ...
    <div class="row">
        ...
        <div class="col-sm-6 col-md-4 col-lg-3">
            <div class="box position-relative" style="height: 380px">
                <button class="btn btn-none like-btn {{ auth()->check() && $product->likes()->where('user_id', auth()->id())->exists() ? 'liked' : '' }}" 
                    data-id="{{ $product->id }}">
                    <span class="heart">{{ auth()->check() && $product->likes()->where('user_id', auth()->id())->exists() ? '❤️' : '🤍' }}</span> 
                    <span class="text">{{ auth()->check() && $product->likes()->where('user_id', auth()->id())->exists() ? 'Đã thích' : 'Thích' }}</span>
                </button>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            document.querySelectorAll(".like-btn").forEach(button => {
                button.addEventListener("click", function () {
                    ...
                    let isLoggedIn = {{ auth()->check() ? 'true' : 'false' }}; // Kiểm tra đăng nhập từ Laravel
    
                    if (!isLoggedIn) {
                        alert("Bạn cần đăng nhập để thích sản phẩm!");
                        window.location.href = "{{ route('loginForm') }}"; // Chuyển hướng đến trang đăng nhập
                        return;
                    }
                    ...
                    .then(data => {
                        if (data.liked !== undefined) {
                            // Cập nhật emoji và văn bản từ phản hồi của server
                            heart.textContent = data.liked ? '❤️' : '🤍'; // Thay đổi emoji
                            text.innerText = data.liked ? "Đã thích" : "Thích"; // Thay đổi văn bản
                        }
                    })
