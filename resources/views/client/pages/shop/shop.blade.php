            ...
            <div class="col-lg-3 col-md-4 mb-4">
                ...
                <form method="GET" action="{{ route('shop') }}">
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-control" name="price_min" min="0" max="{{ $maxPrice }}" value="{{ request('price_min', 0) }}">
                        <input type="number" class="form-control mt-2" name="price_max" min="0" max="{{ $maxPrice }}" value="{{ request('price_max', $maxPrice) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <ul class="list-unstyled">
                            @foreach($categories as $category)
                                <li>
                                    <input type="checkbox" name="category_id[]" value="{{ $category->id }}" {{ in_array($category->id, request('category_id', [])) ? 'checked' : '' }}> {{ $category->name }}
                                </li>
                            @endforeach
                        </ul>
                    </div>
                    <div class="mb-4">
                        ...
                        <ul class="list-unstyled">
                            @foreach($brands as $brand)
                                <li>
                                    <input type="checkbox" name="brand_id[]" value="{{ $brand->id }}" {{ in_array($brand->id, request('brand_id', [])) ? 'checked' : '' }}> {{ $brand->name }}
                                </li>
                            @endforeach
                        </ul>
                    </div>
